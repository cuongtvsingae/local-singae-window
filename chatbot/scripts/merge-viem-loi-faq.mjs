...
const sourcePath = path.join(dataDir, 'viem-loi-faq-import-source.txt');
...
function normalizeText(value) {
  return cleanCell(value)
    .replace(/trình trình/gi, 'tình trạng')
    .replace(/\s+\./g, '.')
    .replace(/\s+,/g, ',')
    .trim();
}
...
if (data.some((item) => String(item.id).startsWith('VIEMLOI-'))) {
  console.error('Refusing to run: VIEMLOI-* entries already exist.');
  process.exit(1);
}

for (const row of uniqueRows) {
  data.push({
    id: `VIEMLOI-${String(row.stt).padStart(3, '0')}`,
    category: 'Dieu tri viem loi',
    question: row.question,
    answer: row.answer,
    keywords: `viem loi;viem nha chu;sales;viemloi-${String(row.stt).padStart(3, '0')}`,
    conditions: '',
    channel_scope: 'all',
    priority: 4,
...
console.log('Appended', uniqueRows.length, 'VIEMLOI entries.');
