...
async function ensureEmbeddingsReadyOrFail() {
	// Prefer local cache first (works offline / without SHEETS_ID in .env)
	...
	if (!config.sheets.id) {
		...
		console.warn(
			'[BOOT] SHEETS_ID not set — skipping Google Sheets embeddings build. IT Support RAG search will be empty until you set SHEETS_ID or add tools/it-support/data/embeddings.json'
		);
		return true;
	}
