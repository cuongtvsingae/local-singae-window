...
export default function AdminDashboardPage() {
  ...
  const loadUsers = async () => {
    setError("");
    const res = await fetch(buildApiAdminUrl("/users"), fetchOpts);
  ...
  const loadTables = async () => {
    const res = await fetch(buildApiAdminUrl("/db/tables"), fetchOpts);
  ...
  const loadChat = async (uid: number) => {
    const res = await fetch(buildApiAdminUrl(`/chat/${uid}`), fetchOpts);
  ...
  const loadTableRows = async (t: string) => {
    const res = await fetch(buildApiAdminUrl(`/db/table/${encodeURIComponent(t)}?limit=50&offset=0`), fetchOpts);
  ...
  const loadLogs = async () => {
    const res = await fetch(buildApiAdminUrl("/logs"), fetchOpts);
  ...
  const loadLogDetail = async (id: number) => {
    const res = await fetch(buildApiAdminUrl(`/logs/${id}`), fetchOpts);
    const j = (await res.json()) as ApiResp<any>;
  ...
  const loadZaloApis = async () => {
    const res = await fetch(buildApiAdminUrl("/zalo/apis"), fetchOpts);
